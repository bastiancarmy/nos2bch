...
window.nostr = {
  ...
  _call(type, params) {
    ...
    console.log(
      "%c[nos2bch:%c" + id + "%c]%c calling %c" + type + "%c with %c" + JSON.stringify(params || {}),
...
window.addEventListener("message", (message) => {
  if (!message.data || message.data.response === null || message.data.response === void 0 || message.data.ext !== "nos2bch" || !window.nostr._requests[message.data.id])
    return;
  ...
  console.log(
    "%c[nos2bch:%c" + message.data.id + "%c]%c result: %c" + JSON.stringify(
